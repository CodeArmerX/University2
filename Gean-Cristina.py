...
def configurar_salas():
    ...
    try:
        while True:
            room_name = input("Nombre de la sala (o * para salir): ")
            if room_name.lower() == "*":
                break  
            room_size = int(input("Tamaño de la sala (nxn): "))
            pelicula = input("Película a proyectar: ")
            date = input("Fecha y hora de proyección: ")
            description = input("Descripción de la sala: ")
            sala = {  
                "nombre": room_name,
                "tamano": room_size,
                "pelicula": pelicula,
                ...
                "asientosDisponibles": [["O" for _ in range(room_size)] for _ in range(room_size)]
            }
            salas.append(sala)
        return salas
...
def reservar_asientos(salas):
    try:
        ...
        for i, sala in enumerate(salas):
            print(str(i + 1) + ". " + sala["nombre"] + " - " + sala["pelicula"] + " - " + sala["date"])
        ...
        room_size = salaSeleccionada["tamano"]
        ...
        try:
            ...
            if fila < 0 or fila >= room_size or columna < 0 or columna >= room_size:
...
def ver_reportes(salas):
    try:
        ...
        for sala in salas:
            ...
            print("Fecha y Hora:", sala["date"])  
            print("Descripción:", sala["description"])  
            print("\n")  
